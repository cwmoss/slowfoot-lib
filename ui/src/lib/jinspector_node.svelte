...
{#if type && data}
    {#if type=='object'}
        <li class="object" class:collapsed>
            <a  on:click="{()=>{console.log("cklick");collapsed=!collapsed}}" href="javascript:;"><strong>{key}</strong></a>
            <ul>
                {#each Object.entries(data) as [k, value]}
            <svelte:self data={value} parent={data} key={k}></svelte:self>
            {/each}
            </ul>
        </li>
    {:else if type=='array'}
    ...
    {:else if type=='string'}
         <li class={type}><strong>{key}</strong><span>{data}</span></li>
    {/if}
{/if}
